...
let complete_class_signature loc env virt kind sign =
  let implicit_public, implicit_declared =
    Ctype.complete_class_signature env sign
  in
  if implicit_public <> [] then begin
...
and class_signature virt env {pcsig_self=sty; pcsig_fields=psign} self_scope loc =
...
let add_instance_var_val_and_par loc sign label id val_env par_env =
  let ty =
    match Vars.find label sign.csig_vars with
    | (_, _, ty) -> ty
    | exception Not_found -> assert false
  in
  add_value_val_and_par loc id ty val_env par_env

let add_instance_vars_val_and_par loc sign vars val_env par_env =
  List.fold_left
    (fun (val_env, par_env) (label, id) ->
       add_instance_var_val_and_par loc sign label id val_env par_env)
    (val_env, par_env) vars
...
let add_instance_var_met loc sign cl_num attrs label id met_env =
  let mut, ty =
    match Vars.find label sign.csig_vars with
    | (mut, _, ty) -> mut, ty
    | exception Not_found -> assert false
  ...
  add_value_met loc id kind ty attrs met_env

let add_instance_vars_met loc sign cl_num vars met_env =
  List.fold_left
    (fun met_env (label, id) ->
       add_instance_var_met loc sign cl_num [] label id met_env)
    met_env vars
  ...
  | Pcf_inherit (override, sparent, super) ->
      Builtin_attributes.warning_scope cf.pcf_attributes
        (fun () ->
           ...
           let inherited_vars =
             Vars.fold
               (fun label _ inherited_vars ->
                  let id = Ident.create_local label in
                  (label, id) :: inherited_vars)
               parent_sign.csig_vars []
           in
           let val_env, par_env =
             add_instance_vars_val_and_par loc sign
               inherited_vars val_env par_env
           in
           let vars =
             List.fold_left
               (fun vars (label, id) -> Vars.add label id vars)
               vars inherited_vars
           in
           let meths =
             Meths.fold
             ...
             | Some {txt=name} ->
                 let (_id, val_env, par_env) =
                   enter_value_val_and_par
                     sparent.pcl_loc name (Btype.self_type parent.cl_type)
                     val_env par_env
                 in
  ...
  | Pcf_val (label, mut, Cfk_virtual styp) ->
      Builtin_attributes.warning_scope cf.pcf_attributes
        (fun () ->
           ...
           let already_declared, id, vars =
             match Vars.find label.txt vars with
             | id -> true, id, vars
             | exception Not_found ->
                 let id = Ident.create_local label.txt in
                 let vars = Vars.add label.txt id vars in
                 false, id, vars
           in
           let val_env, par_env =
             add_instance_var_val_and_par loc sign label.txt id val_env par_env
           in
           let field =
             Virtual_val
  ...
  | Pcf_val (label, mut, Cfk_concrete (override, sdefinition)) ->
      Builtin_attributes.warning_scope cf.pcf_attributes
        (fun () ->
           ...
           in
           let val_env, par_env =
             add_instance_var_val_and_par loc sign
               label.txt id val_env par_env
           in
           let field =
             Concrete_val
...
and class_field_second_pass cl_num sign met_env field =
  ...
  | Inherit { override; parent; super;
      ...
      let met_env =
        add_instance_vars_met loc sign cl_num inherited_vars met_env
      in
      let met_env =
        match super with
        | None -> met_env
        | Some name ->
            ...
            let _id, met_env =
              enter_value_met ~check:(fun s -> Warnings.Unused_ancestor s)
                loc name (Val_anc (sign, super_meths, cl_num))
                (Btype.self_type parent.cl_type) [] met_env
            in
            met_env
      in
      let desc =
  ...
  | Virtual_val { label; mut; id; cty; already_declared; loc; attributes } ->
      let met_env =
        if already_declared then met_env
          ...
          add_instance_var_met loc sign cl_num
            attributes label.txt id met_env
        end
      in
...
and class_structure cl_num virt self_scope final val_env met_env loc
  ...
  in
  let self_value = Val_self (sign, self_var_kind, vars, cl_num) in
  let met_env =
    List.fold_right

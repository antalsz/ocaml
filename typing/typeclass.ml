...
type error =
  ...
  | Self_clash of Ctype.Unification_trace.t
...
let rec class_type_field env self_type arg ctf =
  Builtin_attributes.warning_scope ctf.pctf_attributes
    (fun () -> class_type_field_aux env self_type arg ctf)

and class_type_field_aux env self_type (fields, val_sig, inher, meths) ctf =
  ...
  match ctf.pctf_desc with
    Pctf_inherit sparent ->
      let parent = class_type env Virtual sparent in
      let inher =
...
and class_signature virt env {pcsig_self=sty; pcsig_fields=sign} loc =
  ...
  let (rev_fields, val_sig, inher, meths) =
    Builtin_attributes.warning_scope []
      (fun () ->
         List.fold_left (class_type_field env self_type)
...
and class_type env virt scty =
  Builtin_attributes.warning_scope scty.pcty_attributes
    (fun () -> class_type_aux env virt scty)

and class_type_aux env virt scty =
  let cltyp desc typ =
    {
     cltyp_desc = desc;
     cltyp_type = typ;
  ...
  | Pcty_signature pcsig ->
      let clsig = class_signature virt env pcsig scty.pcty_loc in
  ...
  | Pcty_arrow (l, sty, scty) ->
      ...
      let clty = class_type env virt scty in
      let typ = Cty_arrow (l, ty, clty.cltyp_type) in
      cltyp (Tcty_arrow (l, cty, clty)) typ

  | Pcty_open (od, e) ->
      let (od, newenv) = !type_open_descr env od in
      let clty = class_type newenv virt e in
      cltyp (Tcty_open (od, clty)) clty.cltyp_type

  | Pcty_extension ext ->
      raise (Error_forward (Builtin_attributes.error_of_extension ext))

let class_type env virt scty =
  delayed_meth_specs := [];
  let cty = class_type env virt scty in
...
let enter_value_val_and_par loc label ty val_env par_env =
  let vd =
    { val_type = ty;
      ...
      Types.val_loc = loc; }
  in
  let (id, val_env) = Env.enter_value label vd val_env in
  let par_env = Env.add_value id vd par_env in
  id, val_env, par_env

let add_value_val_and_par loc id ty val_env par_env =
  let vd =
    { val_type = ty;
      val_kind = Val_unbound Val_unbound_instance_variable;
      val_attributes = [];
      Types.val_loc = loc; }
  in
  let val_env = Env.add_value id vd val_env in
  let par_env = Env.add_value id vd par_env in
  val_env, par_env

let enter_value_met ?check loc label kind ty attrs met_env =
  Env.enter_value ?check label
    {val_type = ty; val_kind = kind;
     val_attributes = attrs;
     Types.val_loc = loc} met_env

let add_value_met ?check loc id kind ty attrs met_env =
...
let enter_instance_var loc inherited label mut virt ty val_env par_env vars =
  ...
  let already_declared, id, val_env, par_env =
    ...
    | None, _ | _, true ->
        let id, val_env, par_env =
          enter_value_val_and_par loc label ty val_env par_env
        in
...
let rec class_field_first_pass self_loc cl_num self_type acc cf =
  ...
  | Pcf_inherit (override, sparent, super) ->
      Builtin_attributes.warning_scope cf.pcf_attributes
        (fun () ->
           let parent = class_expr cl_num val_env par_env Virtual sparent in
           ...
           let (val_env, par_env, super) =
             ...
             | Some {txt=name} ->
                 let (_id, val_env, par_env) =
                   enter_value_val_and_par
                     sparent.pcl_loc name self_type val_env par_env
                 in
      ...
      Builtin_attributes.warning_scope cf.pcf_attributes
        (fun () ->
           ...
           begin
             try Ctype.unify val_env ty ty' with Ctype.Unify trace ->
               raise(Error(loc, val_env,
                           Field_type_mismatch ("method", label.txt, trace)));
           end;
           let field =
...
and class_fields_first_pass self_loc cl_num self_type
      val_env par_env cfs =
  let rev_fields = [] in
  ...
  let acc =
    Builtin_attributes.warning_scope []
      (fun () ->
        List.fold_left
          (class_field_first_pass self_loc cl_num self_type)
          init_acc cfs)
  in
      ...
      let met_env =
        List.fold_left
          (fun met_env (_, id, mut, ty) ->
             add_value_met loc id
               (Val_ivar (mut, cl_num)) ty [] met_env)
  ...
  | Virtual_val { label; mut; id; cty; already_declared; loc; attributes } ->
      let met_env =
        if already_declared then met_env
          ...
          add_value_met loc id
            (Val_ivar (mut, cl_num)) cty.ctyp_type attributes met_env
        end
      in
  ...
  | Concrete_val { label; mut; id; override;
      ...
      let met_env =
        if already_declared then met_env
          ...
          add_value_met loc id
            (Val_ivar (mut, cl_num)) definition.exp_type attributes met_env
        end
      in
...
and class_structure cl_num virt final val_env met_env loc
  ...
  let val_env, par_env =
    List.fold_right
      (fun {pv_id; pv_type; pv_loc} (val_env, par_env) ->
         add_value_val_and_par pv_loc pv_id pv_type val_env par_env)
      self_pat_vars (val_env, par_env)
  in
  let public_self = self_pat.pat_type in

  (* Check that the binder has a correct type *)
  let ty =
    match final with
    | Final -> Ctype.newty (Tobject (Ctype.newvar(), ref None))
    | Not_final -> self_type
  in
  ...
  let (fields, inher, vars, meths) =
    class_fields_first_pass self_loc cl_num self_type
           val_env par_env str
  in
  ...
  | Final ->
      ...
      with Ctype.Unify trace -> raise(Error(loc, val_env, Self_clash trace))
      end;
  end;
  ...
  let self_var_kind =
    match virt with
    | Virtual -> Self_virtual(ref self_meths, private_self)
    | Concrete -> Self_concrete self_meths
  in
  let self_value = Val_self (self_var_kind, vars, cl_num) in
  let met_env =
    List.fold_right
      (fun {pv_id; pv_type; pv_loc; pv_as_var; pv_attributes} met_env ->
         let check =
           if pv_as_var then (fun s -> Warnings.Unused_var s)
           else (fun s -> Warnings.Unused_var_strict s)
         in
         add_value_met ~check pv_loc pv_id self_value
           pv_type pv_attributes met_env)
...
and class_expr cl_num val_env met_env virt scl =
  Builtin_attributes.warning_scope scl.pcl_attributes
    (fun () -> class_expr_aux cl_num val_env met_env virt scl)

and class_expr_aux cl_num val_env met_env virt scl =
  ...
  | Pcl_structure cl_str ->
      let desc =
        class_structure cl_num virt Not_final
  ...
  | Pcl_fun (l, Some default, spat, sbody) ->
      ...
      in
      class_expr cl_num val_env met_env virt sfun
  | Pcl_fun (l, None, spat, scl') ->
      ...
      in
      Ctype.raise_nongen_level ();
      let cl = class_expr cl_num val_env' met_env virt scl' in
      Ctype.end_def ();
  ...
  | Pcl_constraint (scl', scty) ->
      ...
      let context = Typetexp.narrow () in
      let clty = class_type val_env virt scty in
  ...
  | Pcl_open (pod, e) ->
      ...
      let cl = class_expr cl_num new_val_env new_met_env virt e in
...
let class_declaration env virt sexpr =
  incr class_num;
  let expr = class_expr (Int.to_string !class_num) env env virt sexpr in
  complete_class_type expr.cl_loc env virt Class expr.cl_type;
  (expr, expr.cl_type)

let class_description env virt sexpr =
  let expr = class_type env virt sexpr in
...
let type_object env loc s =
  incr class_num;
  let desc =
    class_structure (Int.to_string !class_num) Concrete Final env env loc s
  in
...
let report_error env ppf = function
  ...
  | Virtual_class (kind, mets, vals) ->
      ...
      let print_msg ppf =
        match kind with
        | Object -> fprintf ppf "This object has virtual %s" missings
        | Class -> fprintf ppf "This class should be virtual"
        | Class_type -> fprintf ppf "This class type should be virtual"
      in
      fprintf ppf
  ...
  | Undeclared_methods(kind, mets) ->
      let message =
        match kind with
        | Object -> "This object has undeclared virtual methods."
        | Class -> "This class has undeclared virtual methods."
        | Class_type -> "This class type has undeclared virtual methods."
      in
      fprintf ppf
        "@[%s@ @[<2>The following virtual methods were not declared : %a@]@]"
        message (pp_print_list ~pp_sep:pp_print_space pp_print_string) mets

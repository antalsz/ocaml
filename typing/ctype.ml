...
let closed_class params sign =
  List.iter mark_type params;
  mark_type_node (repr sign.csig_self_row);
  try
    Meths.iter
      (fun lab (priv, _, ty) ->
        if priv = Public then
...
let rec limited_generalize_class_type rv =
  function
    Cty_constr (_path, params, cty) ->
      List.iter (limited_generalize rv) params;
      limited_generalize_class_type rv cty
  | Cty_signature sign ->
      limited_generalize rv sign.csig_self;
      limited_generalize rv sign.csig_self_row;
...
let unify_self_types env sign1 sign2 =
  ...
  | () -> ()
  | exception Unify trace -> begin
      match trace with
      | Trace.Diff _ :: Trace.Incompatible_fields {name; _} :: rem ->
          let failure = Method (name, Type_mismatch rem) in
          raise (Inherit_class_signature_failed failure)
      | _ ->
          raise (Inherit_class_signature_failed (Self_type_mismatch trace))
    end
...
let complete_class_signature env sign =
  ...
  sign.csig_meths <- meths;
  sign.csig_self_row <- row;
  hide_private_methods self;
  implicit_public, implicit_declared
...
let rec moregen_clty trace type_pairs env cty1 cty2 =
  try
    ...
    | Cty_signature sign1, Cty_signature sign2 ->
        Meths.iter
          (fun lab (_priv, _v, ty) ->
             let (_priv', _v', ty') = Meths.find lab sign1.csig_meths in
...
let match_class_types ?(trace=true) env pat_sch subj_sch =
  ...
  let error =
    Meths.fold
      (fun lab (priv, vr, _) err ->
         ...
         | (priv', vr', _) ->
             match priv', vr', priv, vr with
             | Public, _, Private, _ -> CM_Public_method lab::err
             | _, Virtual, _, Concrete -> CM_Virtual_method lab::err
             | _, _, _, _ -> err)
      sign2.csig_meths error
  in
  let error =
    Meths.fold
      (fun lab (priv, vr, _) err ->
  ...
  let error =
    Vars.fold
      (fun lab (mut, vr, _) err ->
         ...
         | (mut', vr', _) ->
             match mut', vr', mut, vr with
             | Immutable, _, Mutable, _ -> CM_Non_mutable_value lab::err
             | _, Virtual, _, Concrete -> CM_Non_concrete_value lab::err
             | _, _, _, _ -> err)
  ...
  | [] ->
      ...
      *)
      ...
      let self1 = repr sign1.csig_self in
      let self2 = repr sign2.csig_self in
      let row1 = repr sign1.csig_self_row in
      let row2 = repr sign2.csig_self_row in
      ...
      let res =
        ...
        | () -> []
        | exception Failure r -> r
      in
      current_level := old_level;
      res
  | error ->
      CM_Class_type_mismatch (env, pat_sch, subj_sch)::error
...
let rec equal_clty trace type_pairs subst env cty1 cty2 =
  try
    ...
    | Cty_signature sign1, Cty_signature sign2 ->
        Meths.iter
          (fun lab (_, _, ty) ->
             let (_, _, ty') = Meths.find lab sign1.csig_meths in
    ...
    Meths.fold
      (fun lab (priv, vr, _) err ->
         ...
         | (priv', vr', _) ->
              match priv', vr', priv, vr with
              | Public, _, Private, _ -> CM_Public_method lab::err
              | Private, _, Public, _ -> CM_Private_method lab::err
              | _, Virtual, _, Concrete -> CM_Virtual_method lab::err
              | _, _, _, _ -> err)
      sign2.csig_meths error
  in
...
let rec nongen_schema_rec env ty =
  ...
  if TypeSet.mem ty !visited then () else begin
    ...
    | Tvariant row ->
        ...
        if not (static_row row) then nongen_schema_rec env row.row_more
    | _ ->

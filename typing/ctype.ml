...
let enter_poly_unif env univar_pairs t1 tl1 t2 tl2 f =
  try
    enter_poly env univar_pairs t1 tl1 t2 tl2 f
  with Escape e -> raise (Unify [Escape e])
...
let enter_poly_eql env univar_pairs t1 tl1 t2 tl2 f =
  try
    enter_poly env univar_pairs t1 tl1 t2 tl2 f
  with Escape e -> raise (Equality [Escape e])
      ...
      match ty.desc with
        Tvar o ->
          ...
          if ty.level < fresh_constr_scope then
            raise (Unify [Escape(escape (Constructor path))])
      | Tvariant r ->
          let r = row_repr r in
      ...
      with Not_found ->
        ...
        *)
        ...
        | (Tpoly (t1, tl1), Tpoly (t2, tl2)) ->
            enter_poly_unif env univar_pairs
...
let unify_occur env t1 t2 =
  try
    occur env t1 t2
  with Occur -> raise (Unify[Unification.Rec_occur(t1, t2)])
...
let unify_update_scope scope ty =
  try
    update_scope scope ty
  with Escape e -> raise (Unify [Escape e])
...
let unify1_var env t1 t2 =
  assert (is_Tvar t1);
  unify_occur env t1 t2;
  unify_occur_univar env t2;
  let d1 = t1.desc in
  link_type t1 t2;
  try
...
let rec unify (env:Env.t ref) t1 t2 =
  ...
  try
    ...
    | (Tunivar _, Tunivar _) ->
        ...
        unify_update_scope t1.scope t2;
        link_type t1 t2
...
and unify2 env t1 t2 =
  ...
  unify_update_scope scope t2;
  unify_update_scope scope t1;
  if unify_eq t1' t2' then () else
...
and unify3 env t1 t1' t2 t2' =
  ...
  | (Tvar _, _) ->
      ...
      link_type t1' t2;
  | (_, Tvar _) ->
      unify_occur !env t2' t1;
      unify_occur_univar !env t1;
      link_type t2' t1;
      ...
      | (Tfield(f,kind,_,rem), Tnil) | (Tnil, Tfield(f,kind,_,rem)) ->
          ...
          | _      ->
              if f = dummy_method then
                raise (Unify Unification.[Obj Self_cannot_be_closed])
              else if d1 = Tnil then
                raise (Unify Errortrace.[Obj(Missing_field (First, f))])
              else
                raise (Unify Errortrace.[Obj(Missing_field (Second, f))])
          end
      | (Tnil, Tnil) ->
          ()
      | (Tpoly (t1, []), Tpoly (t2, [])) ->
          unify env t1 t2
      | (Tpoly (t1, tl1), Tpoly (t2, tl2)) ->
          enter_poly_unif !env univar_pairs t1 tl1 t2 tl2 (unify env)
      ...
      (fun (n, k1, t1, k2, t2) ->
        ...
        try
          if !trace_gadt_instances then begin
            ...
            unify_update_scope va.scope t1
          end;
          unify env t1 t2
        with Unify trace ->
...
and unify_row env row1 row2 =
  ...
  let set_more row rest =
    ...
    if row_fixed row then
      ...
      unify_update_scope rm.scope ty;
      link_type rm ty
  in
...
and unify_row_field env fixed1 fixed2 more l f1 f2 =
  ...
  let if_not_fixed (pos,fixed) f =
    ...
    | None -> f ()
    | Some fix ->
        let tr = [Unification.Variant(Fixed_row(pos,Cannot_add_tags [l],fix))] in
        raise (Unify tr) in
  ...
  | Reither(c1, tl1, m1, e1), Reither(c2, tl2, m2, e2) ->
      ...
      let split_univars =
        List.partition
          (fun ty -> try occur_univar !env ty; true with Escape _ -> false) in
  ...
  | Reither(false, tl, _, e1), Rpresent(Some t2) ->
      if_not_fixed first (fun () ->
          ...
          unify_update_scope rm.scope t2;
  ...
  | Rpresent(Some t1), Reither(false, tl, _, e2) ->
      if_not_fixed second (fun () ->
          ...
          unify_update_scope rm.scope t1;
...
let filter_method env name priv ty =
  ...
  match ty.desc with
    Tvar _ ->
      ...
      unify_update_scope ty.scope ty';
      link_type ty ty';
    ...
    | _ ->
        ...
        with Not_found ->
          ...
          | (Tpoly (t1, []), Tpoly (t2, [])) ->
              moregen inst_nongen type_pairs env t1 t2
          | (Tpoly (t1, tl1), Tpoly (t2, tl2)) ->
              enter_poly_moregen env univar_pairs t1 tl1 t2 tl2
...
and moregen_row inst_nongen type_pairs env row1 row2 =
  ...
  if row1.row_closed then begin
    ...
    | false, _ -> raise (Moregen [Variant Openness])
  ...
  | _ when may_inst ->
      ...
      moregen_update_scope rm1.scope ext;
      link_type rm1 ext
  | Tconstr _, Tconstr _ ->
    ...
    | _ ->
        ...
        with Not_found ->
          ...
          | (Tpoly (t1, []), Tpoly (t2, [])) ->
              eqtype rename type_pairs subst env t1 t2
          | (Tpoly (t1, tl1), Tpoly (t2, tl2)) ->
              enter_poly_eql env univar_pairs t1 tl1 t2 tl2
  ...
  let rec check_cycle seen ty =
    ...
    match ty.desc with
      Tconstr (p, _tl, _abbrev) ->
        ...
        begin try
          check_cycle (ty :: seen) (try_expand_safe_opt env ty)
              ...
              let tyl' =
                List.fold_left
                  (fun tyl ty ->
                     if List.exists
                          (fun ty' ->
                             match equal env false [ty] [ty'] with
                             | () -> true
                             | exception Equality _ -> false)
                          tyl
  ...
  with Not_found ->
    ...
    ty'.desc <-
      ...
      | Tconstr(p, tl, _abbrev) ->
          ...
          | Some id -> begin
              match try_expand env (newty2 ty.level ty.desc) with
              | ty' ->
                  Tlink (nondep_type_rec ~expand_private env ids ty')
                  (*
                     ...
                     description.
                   *)
              | exception Cannot_expand -> raise (Nondep_cannot_erase id)
            end
          | None ->
